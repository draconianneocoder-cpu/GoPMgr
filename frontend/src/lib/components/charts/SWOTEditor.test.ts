...
import SWOTEditor from './SWOTEditor.svelte';
...
// not cover quadrant CRUD, layout rendering, or autosave.
//
...
function installApp(overrides: Partial<AppMock> = {}): AppMock {
  const chart = {
    ...
    kind: 'swot',
    title: 'SWOT',
    data: JSON.stringify({ strengths: [], weaknesses: [], opportunities: [], threats: [] }),
  };
  const app: AppMock = {
    GetChart: vi.fn(async () => chart),
    SaveChart: vi.fn(async (c: unknown) => c),
    LayoutChart: vi.fn(async () => ({ body: { quadrants: [] } })),
    ...overrides,
  };
...
describe('SWOTEditor migrated header "&larr; Dashboard" button', () => {
  it('Button variant="nav"', async () => {
    const app = installApp();
    const { getByText } = render(SWOTEditor);
...
describe('SWOTEditor migrated load-error "Back to dashboard" button', () => {
  it('Button variant="primary" size="md"', async () => {
    installApp({ GetChart: vi.fn(async () => { throw new Error('boom'); }) });
    const { findByText } = render(SWOTEditor);
