...
import SigmaWorkspace from './SigmaWorkspace.svelte';

let app: Record<string, ReturnType<typeof vi.fn>>;

beforeEach(() => {
  app = {
    SigmaListProjects: vi.fn(async () => []),
  };
...
// proven by the assertion passing rather than throwing. The primary load
// mock (SigmaListProjects) is asserted first so a silently-swallowed load
// failure (caught internally and routed to loadError, not thrown) would
// surface as a timeout here rather than a false pass.
describe('SigmaWorkspace migrated header "&larr; Dashboard" button', () => {
  it('Button variant="nav"', async () => {
    const { getByText } = render(SigmaWorkspace);
    await waitFor(() => expect(app.SigmaListProjects).toHaveBeenCalled());
