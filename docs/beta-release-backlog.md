...
| --- | --- | --- |
| Prevent silent editor data loss | In progress | Native workflows preserve unsaved changes on navigation, project close, sign-out, and window close; failed saves remain visible and block unsafe continuation. 2026-08-10: the Kanban/Backlog work-item editor modal's four close paths (header ×, Cancel, Escape, backdrop click) now confirm before discarding an unsaved edit — see `.agent_memory/workitem-editor-discard-guard-2026-08-10.md` and `frontend/src/lib/components/agile/WorkItemEditor.test.ts`. `SprintList.svelte`'s sprint editor modal has the same unguarded shape and is not yet fixed. Native project-close/sign-out/window-close still need real packaged-app evidence, not just source inspection. |
