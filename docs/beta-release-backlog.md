...
| --- | --- | --- |
| Prevent silent editor data loss | In progress | Native workflows preserve unsaved changes on navigation, project close, sign-out, and window close; failed saves remain visible and block unsafe continuation. 2026-08-10: the Kanban/Backlog work-item editor modal and the sprint editor modal both had all four close paths (header ×, Cancel, Escape, backdrop click) discard an unsaved edit with no confirmation; both now confirm before discarding — see `.agent_memory/workitem-editor-discard-guard-2026-08-10.md`, `.agent_memory/sprintlist-discard-guard-2026-08-10.md`, `frontend/src/lib/components/agile/WorkItemEditor.test.ts`, and `frontend/src/lib/components/agile/SprintList.test.ts`. Native project-close/sign-out/window-close still need real packaged-app evidence, not just source inspection. |
