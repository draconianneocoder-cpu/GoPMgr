...
| --- | --- | --- |
| Prevent silent editor data loss | In progress | The shared guard protects registered editors on navigation, project close, sign-out, and window close; failed saves and edits made during a save remain visible and block unsafe continuation. The 2026-08-15 native-close guard freezes renderer interaction after a clean, saved, or discarded decision, then has Go grant and consume its one-shot permit while invoking Wails quit; `main_test.go` and `frontend/src/lib/native-close.test.ts` cover the source contract. `WorkItemEditor` and `SprintList` each register only their current open draft, with timed auto-save disabled because a successful save closes their modal. In both, a late modal-field edit survives the first save, retains backend-owned fields, and requires another successful save before close. `StakeholderManager` remains an unregistered native-close P0 gap. Native project-close/sign-out/window-close still need real packaged-app evidence, not just source inspection. |
