...
(RFC 9180: DHKEM(X25519, HKDF-SHA256), HKDF-SHA256, AES-256-GCM). No new
dependency.
...
- **Escrow pin.** When an account first enrolls, it stores a MAC of the escrow
  key ID and public key under a subkey of its own DEK. Every later seal at
...
- **Personal public keys.** At sign-in, an account derives its public key from
  ...
  under a subkey of the escrow private key. At account creation this happens
...
- **Remaining window, trust on first use.** An existing account's first
  ...
  attestation of an existing account's personal key trusts that key. The owner
  must accept this window.
...
| Demotion, disable, or permanent deletion of an administrator | Delete that administrator's grant (`ON DELETE CASCADE` on the account row). A disabled administrator cannot sign in, but could otherwise still decrypt offline. |
...
`personal_key_repaired`, and `escrow_rotated`. `account_events` has not shipped
in a release yet. Changing its `CHECK` after a release needs a table rebuild,
so this change should land before it ships.

### Access flow

1. The administrator chooses a user in the Admin panel and types a reason.
...
4. The key lives in a separate, read-scoped handle, never in `a.dek`. The
...
5. The handle covers the target's projects. Whether it also covers their
...
1. **Read-only or read-write access?** Recommended: read-only. With read-write,
   changes made while viewing would be recorded in the user's project audit
   trail under the administrator's name, and the owner must say whether the
   user can see them.
