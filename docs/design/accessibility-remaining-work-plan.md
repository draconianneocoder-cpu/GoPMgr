...
**Status:** Planning document, 2026-08-24; updated 2026-09-04 — M1 (native
...
| Stats-family canvas theme | Shipped; configuration-tested 2026-09-04 | `StatsChart.svelte` now reads the active slate CSS variables, rebuilds on `data-theme` changes, and applies them to titles, legends, axes, grid lines, pie borders, and default annotation lines. `stats_chart_theme.test.ts` checks light-theme token parsing and the actual Chart.js configuration, including the Pareto cumulative series and 80% annotation sharing the right percentage axis. | No native-window visual pass was run for this correction. Other custom SVG/canvas chart renderers do not use this shared configuration and remain part of a future app-wide visual audit. |
