...
| Opening a pre-Cost-Control project adds its exact-money/currency and Cost Control schema additively; legacy project currency defaults to USD without conversion, and plaintext-to-encrypted migration preserves that state while retaining a protected local plaintext backup | `internal/db/sqlite.go`, `internal/db/encryption.go`, `internal/db/cost_control_migration_test.go` |
