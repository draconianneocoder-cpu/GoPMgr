...
| Reusable supplier and item records are a signed-in-user convenience catalog, encrypted separately from both pre-login `system.db` and project files. Records are versioned and archived; later project-ledger rows must retain only display snapshots needed for historical export, never supplier contact PII. | [Reusable procurement catalog](../docs/design/reusable-procurement-catalog.md), `internal/catalog`, `app_catalog.go`, `frontend/src/lib/components/project/ProcurementCatalog.svelte` |
