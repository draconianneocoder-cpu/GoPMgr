...
- **Risk/Issue/Opportunity workflow + Risk Matrix chart** (22nd chart kind) — _foundation complete 2026-07-25._ `matrix.RiskMatrixDocument` stores risks, active issues, and opportunities with probability/impact, owner, status, mitigation, and linked task. The backend emits a validated canonical 5×5 grid; the dedicated Svelte editor and vector PDF renderer consume that layout, and Risk Register documents can link the chart into combined reports. Remaining: optional document/chart row synchronization so users do not need to maintain parallel records.
