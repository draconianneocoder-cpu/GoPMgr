...
| Reusable supplier and item records are a signed-in-user convenience catalog, encrypted separately from both pre-login `system.db` and project files. Catalog Wails JSON is snake_case. Cost Control queries active records and copies only item name/SKU/default unit or supplier display name into editable project-ledger snapshots; it retains no catalog ID/version or live cross-database relation. Automatic catalog copying excludes supplier contact PII, while arbitrary manual free text remains outside that guarantee. Attachment ZIP export binds each manifest archive name to its output entry and rechecks stored byte count/SHA-256 before writing that entry. | [Reusable procurement catalog](../docs/design/reusable-procurement-catalog.md), `internal/catalog`, `app_catalog.go`, `app_catalog_test.go`, `app_cost_control.go`, `app_cost_control_attachments.go`, `internal/db/cost_control.go`, `internal/db/cost_control_attachments.go`, `internal/export/attachments_zip.go`, `frontend/src/lib/components/project/ProcurementCatalog.svelte`, `frontend/src/lib/components/project/CostControlPanel.svelte`, `frontend/src/lib/components/project/CostControlPanel.test.ts`, `internal/db/cost_control_procurement_test.go`, `internal/db/cost_control_attachments_test.go`, `app_cost_control_attachments_test.go`, `internal/export/attachments_zip_test.go` |
