...
// (GOPMGR_DIALOG_TITLE / GOPMGR_DIALOG_MESSAGE) rather than embedded in
...
func ShowError(title, message string) {
	const command = `Add-Type -AssemblyName PresentationFramework; ` +
		`[System.Windows.MessageBox]::Show($env:GOPMGR_DIALOG_MESSAGE, $env:GOPMGR_DIALOG_TITLE, 'OK', 'Error') | Out-Null`
	...
	cmd.Env = append(cmd.Environ(),
		"GOPMGR_DIALOG_TITLE="+title,
		"GOPMGR_DIALOG_MESSAGE="+message,
	)
	_ = cmd.Run()
}
