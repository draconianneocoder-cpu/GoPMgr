...
Local multi-user account system: Argon2id-backed accounts, ADR-001 DEK hierarchy, PMForge→GoPMgr data-root migration. Coverage sweep in progress (2026-08-05): `dek.go` closed to 90.2%; `recovery.go` closed to 92.9% (`IssueRecoveryCodes` 94.7%, `ResetWithRecoveryCode` 92.2%, `migrateRecoveryTable`/`canonicalise` 100%); `store.go` closed to 88.2% in its first of two sub-increments — `Open`/`Close`/`RootDir`/`migrate`/`migrateAdminColumn`/`SetAdmin`/`DeleteAccount`/`CreateAccount`/`Authenticate`/`List` (all DB-backed, `CreateAccount`/`Authenticate`/`List` now 100%), plus `ensurePrivateSQLiteFiles`'s one forceable branch (`ensurePrivateDir` was already at 100% as a side effect of `Open`'s root-as-file test); the second sub-increment (`MigrateLegacyRoot`/`migrateLegacyRoot`/`copyTree`/`copyFile`, all filesystem-backed) is the final remaining increment for the whole package.
