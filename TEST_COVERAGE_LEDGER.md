...
Local multi-user account system: Argon2id-backed accounts, ADR-001 DEK hierarchy, PMForge→GoPMgr data-root migration. Coverage sweep in progress (2026-08-05): `dek.go` closed to 90.2% in this pass; `recovery.go` (70.5%) and `store.go` (75.5%) are the next targets — deliberately split into separate increments given the package's size, matching the `internal/crypto` split precedent.
