...
Local multi-user account system: Argon2id-backed accounts, ADR-001 DEK hierarchy, PMForge→GoPMgr data-root migration. Coverage sweep in progress (2026-08-05): `dek.go` closed to 90.2%; `recovery.go` closed to 92.9% (`IssueRecoveryCodes` 94.7%, `ResetWithRecoveryCode` 92.2%, `migrateRecoveryTable`/`canonicalise` 100%); `store.go` (75.5%) is the final increment — deliberately split, matching the `internal/crypto` split precedent.
