...
- **The four frozen literals**, each intentionally still "PMForge"/`.pmforge` after the rename, and the test that now pins it:
...
- **A same-day follow-up request asked to rename these frozen literals to GoPMgr/gopmgr outright.** Declined to implement without further scoping: the data-root directory name has an existing migration precedent (`MigrateLegacyRoot`), but `.pmforge` and `project.pmforge` name files a migration cannot reach — `.pmforge` project files and `.pmba` archives already exist in arbitrary user-chosen locations (Downloads, external drives, users' own cron jobs, per `HelpGuide.svelte`'s own documented CLI usage), so renaming either requires a permanent dual-extension/dual-entry-name read path, not a one-time copy. `PMForge_Archive_` is the one literal that's genuinely just cosmetic (nothing reads it back) and safe to rename on its own — but note that approving that rename means updating `TestSecureArchiveUsesPMForgeArchivePrefix` (added this entry) in the same change. See `docs/security-quality-review-2026-08-04.md`, "Scope decision explicitly deferred," for the full per-literal decomposition; that decision was still pending as of this writing.
