...
- **A fifth literal, outside the four the user actually decomposed and approved, was also renamed: `internal/export/sigma_report.go`'s `~/PMForge/exports` → `~/GoPMgr/exports`.** Found during a post-implementation sweep for remaining "PMForge"/".pmforge" references, not part of the original decomposition. Judged safe on the same reasoning as `PMForge_Archive_` — it's a generated-PDF output directory, nothing reads it back, so no compatibility branch is needed — but flagged explicitly here because it was *not* something the user was asked about. One real consequence: a user's prior Sigma reports stay at the old path with nothing pointing them there; this is a cosmetic drift, not a data-loss risk, since the PDFs themselves are untouched.
