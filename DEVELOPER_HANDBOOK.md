...
- **The four frozen literals**, each intentionally still "PMForge"/`.pmforge` after the rename, and the test that now pins it:
  - `internal/users/store.go` `DefaultRootDir()` — the data-root directory is `~/Library/Application Support/PMForge` (macOS) or `~/Documents/PMForge` (Linux/Windows/`$XDG_DATA_HOME`). Renaming the leaf *or* swapping the macOS parent (e.g. back to `Documents`) both orphan every existing install's accounts and projects. Pinned by `internal/users/root_dir_test.go` (`TestDefaultRootDirXDGOverride_UsesPMForgeLeaf`, `TestDefaultRootDirPlatformDefault_UsesPMForgeLeaf`, which also asserts the full macOS path, not just the leaf).
